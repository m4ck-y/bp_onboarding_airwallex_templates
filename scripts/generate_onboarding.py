...
def generate_txt_with_copany_name(company_name: str, content: str):
    ...
    filename = f"data/{company_name_normalized}.txt"
    save_text_to_file(content, filename)


data: list[schema_airwallex_onboarding] = [

    schema_airwallex_onboarding(
        mongo_data=schema_mongo_lead(
            user_information=schema_user_information(password="#TODO:mongo.password"),
            administration=schema_administration(lead_id="{#TODO:mongo.lead_id}"),
        ),
        ...
        support_info=schema_support_info(
            ...
            outbound_transactions=schema_outbound(
                per_transaction=7000, daily=395000, monthly=470000
            ),
        ),
    ),
...
if __name__ == "__main__":
    for company in data:
        data_str_output = generate_scripts_db(company)
        generate_txt_with_copany_name(company.sumsub_company.comercial_name, data_str_output)
